...
header.user = errors.handler(function (pos, input_allowed)
    ...
    local normalized_text_pos, text_size =
        irender.text(utils.get_username(), fonts.header, text_pos, color:salpha(hover_anim), irender.flags.RIGHT_ALIGN + irender.flags.Y_ALIGN)
...
header.open = errors.handler(function()
    if not dummy_options.open then
        dummy_options.pos = input.cursor_pos()
    end
...
header.get_avatar = errors.handler(function ()
    ...
    once(function ()
        http.download(security.avatar_url, nil, function(path)
            header.avatar_texture = irender.setup_texture(path)
        end)
    end, "get_avatar")
...
header.draw = errors.handler(function (pos, alpha, input_allowed)
    ...
    irender.text("A", fonts.logo, icon_pos, colors.magnolia:alpha(alpha))
    ...
    render.line(line_start_pos, line_start_pos + v2(0, line_size / 2), col.white:alpha(20):salpha(alpha))
