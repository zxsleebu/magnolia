...
errors.report = function(name)
    local code = errors.get()
    local error_name = table.find_by_value(errors, code) or tostring(code)
    print(name .. ": " .. error_name)
    error()
end
errors.check = function(name)
    local err = errors.get()
    if err ~= 0 then
        errors.report(name)
    end
end
